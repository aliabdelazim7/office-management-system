...
--
--  Seeds the 51 permission definitions.
-- =============================================================================
...
SELECT gen_random_uuid()::text, 'backfilled', now(), '20260720130150_init', now(), 1
WHERE EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'tenants')
  AND NOT EXISTS (
    SELECT 1 FROM "_prisma_migrations" WHERE "migration_name" = '20260720130150_init'
  );


-- =============================================================================
--  Migration: 20260720130150_init
-- =============================================================================

DO $mig_20260720130150_init$
BEGIN
  IF EXISTS (
    SELECT 1 FROM "_prisma_migrations" WHERE "migration_name" = '20260720130150_init'
  ) THEN
    RAISE NOTICE 'Skipping 20260720130150_init — already applied.';
    RETURN;
  END IF;
  ...
  INSERT INTO "_prisma_migrations"
    ("id", "checksum", "finished_at", "migration_name", "started_at", "applied_steps_count")
  VALUES (gen_random_uuid()::text, 'a2abc517463903f1698e07960aff615d978807d24d8a489605248d7797df3003', now(), '20260720130150_init', now(), 1);

  RAISE NOTICE 'Applied 20260720130150_init.';
END
$mig_20260720130150_init$;
...
DO $mig_20260721090000_user_permission_overrides$
BEGIN
  IF EXISTS (
    SELECT 1 FROM "_prisma_migrations" WHERE "migration_name" = '20260721090000_user_permission_overrides'
  ) THEN
    RAISE NOTICE 'Skipping 20260721090000_user_permission_overrides — already applied.';
    RETURN;
  END IF;
  ...
  INSERT INTO "_prisma_migrations"
    ("id", "checksum", "finished_at", "migration_name", "started_at", "applied_steps_count")
  VALUES (gen_random_uuid()::text, '429ff2c66a83682e87b0e1f30d8d47037fc292b6c5d8b242a9c3677ce1795d86', now(), '20260721090000_user_permission_overrides', now(), 1);

  RAISE NOTICE 'Applied 20260721090000_user_permission_overrides.';
END
$mig_20260721090000_user_permission_overrides$;


-- =============================================================================
--  Permission catalogue (51 permissions)
-- =============================================================================
...
VALUES
  ('client.view', 'crm', 'إدارة العملاء', 'عرض العملاء', 'View clients', NULL, 0),
  ('client.view_all', 'crm', 'إدارة العملاء', 'عرض كل العملاء', 'View all clients', 'بدون هذه الصلاحية يرى الموظف عملاءه المكلف بهم فقط', 1),
  ('client.create', 'crm', 'إدارة العملاء', 'إضافة عميل', 'Create client', NULL, 2),
  ('client.update', 'crm', 'إدارة العملاء', 'تعديل عميل', 'Update client', NULL, 3),
  ('client.delete', 'crm', 'إدارة العملاء', 'حذف عميل', 'Delete client', NULL, 4),
  ('client.export', 'crm', 'إدارة العملاء', 'تصدير بيانات العملاء', 'Export clients', NULL, 5),
  ('document.view', 'documents', 'المستندات', 'عرض المستندات', 'View documents', NULL, 6),
  ('document.upload', 'documents', 'المستندات', 'رفع مستند', 'Upload document', NULL, 7),
  ('document.update', 'documents', 'المستندات', 'تعديل مستند', 'Update document', NULL, 8),
  ('document.delete', 'documents', 'المستندات', 'حذف مستند', 'Delete document', NULL, 9),
  ('document.download', 'documents', 'المستندات', 'تحميل مستند', 'Download document', NULL, 10),
  ('gov.view', 'government', 'المستندات الحكومية', 'عرض المستندات الحكومية', 'View government records', NULL, 11),
  ('gov.manage', 'government', 'المستندات الحكومية', 'إدارة المستندات الحكومية', 'Manage government records', NULL, 12),
  ('contract.view', 'contracts', 'العقود', 'عرض العقود', 'View contracts', NULL, 13),
  ('contract.manage', 'contracts', 'العقود', 'إدارة العقود', 'Manage contracts', NULL, 14),
  ('contract.template.manage', 'contracts', 'العقود', 'إدارة قوالب العقود', 'Manage contract templates', NULL, 15),
  ('service.view', 'services', 'الخدمات وسير العمل', 'عرض أوامر الخدمة', 'View service orders', NULL, 16),
  ('service.view_all', 'services', 'الخدمات وسير العمل', 'عرض كل أوامر الخدمة', 'View all service orders', 'بدون هذه الصلاحية يرى الموظف المهام المكلف بها فقط', 17),
  ('service.create', 'services', 'الخدمات وسير العمل', 'إنشاء أمر خدمة', 'Create service order', NULL, 18),
  ('service.update', 'services', 'الخدمات وسير العمل', 'تعديل أمر خدمة', 'Update service order', NULL, 19),
  ('service.delete', 'services', 'الخدمات وسير العمل', 'حذف أمر خدمة', 'Delete service order', NULL, 20),
  ('service.step.update', 'services', 'الخدمات وسير العمل', 'تحديث مراحل العمل', 'Update workflow steps', NULL, 21),
  ('service.catalog.manage', 'services', 'الخدمات وسير العمل', 'إدارة كتالوج الخدمات', 'Manage service catalogue', NULL, 22),
  ('user.view', 'hr', 'الموارد البشرية', 'عرض الموظفين', 'View employees', NULL, 23),
  ('user.invite', 'hr', 'الموارد البشرية', 'دعوة موظف', 'Invite employee', NULL, 24),
  ('user.update', 'hr', 'الموارد البشرية', 'تعديل بيانات موظف', 'Update employee', NULL, 25),
  ('user.deactivate', 'hr', 'الموارد البشرية', 'إيقاف موظف', 'Deactivate employee', NULL, 26),
  ('user.salary.view', 'hr', 'الموارد البشرية', 'عرض المرتبات', 'View salaries', NULL, 27),
  ('field.view_own', 'field', 'المهام الميدانية', 'عرض مهامي الميدانية', 'View own field tasks', NULL, 28),
  ('field.view_all', 'field', 'المهام الميدانية', 'عرض كل المهام الميدانية', 'View all field tasks', NULL, 29),
  ('field.assign', 'field', 'المهام الميدانية', 'إسناد مهمة ميدانية', 'Assign field task', NULL, 30),
  ('field.execute', 'field', 'المهام الميدانية', 'تنفيذ مهمة ميدانية', 'Execute field task', NULL, 31),
  ('invoice.view', 'finance', 'الحسابات والمالية', 'عرض الفواتير', 'View invoices', NULL, 32),
  ('invoice.create', 'finance', 'الحسابات والمالية', 'إنشاء فاتورة', 'Create invoice', NULL, 33),
  ('invoice.update', 'finance', 'الحسابات والمالية', 'تعديل فاتورة', 'Update invoice', NULL, 34),
  ('invoice.delete', 'finance', 'الحسابات والمالية', 'حذف فاتورة', 'Delete invoice', NULL, 35),
  ('payment.record', 'finance', 'الحسابات والمالية', 'تسجيل دفعة', 'Record payment', NULL, 36),
  ('expense.view', 'finance', 'الحسابات والمالية', 'عرض المصروفات', 'View expenses', NULL, 37),
  ('expense.manage', 'finance', 'الحسابات والمالية', 'إدارة المصروفات', 'Manage expenses', NULL, 38),
  ('ledger.view', 'finance', 'الحسابات والمالية', 'عرض دفتر الأستاذ', 'View client ledger', NULL, 39),
  ('finance.report', 'finance', 'الحسابات والمالية', 'التقارير المالية', 'Financial reports', NULL, 40),
  ('notification.view', 'notifications', 'التنبيهات والمراسلات', 'عرض التنبيهات', 'View notifications', NULL, 41),
  ('notification.template.manage', 'notifications', 'التنبيهات والمراسلات', 'إدارة قوالب التنبيهات', 'Manage notification templates', NULL, 42),
  ('whatsapp.send', 'notifications', 'التنبيهات والمراسلات', 'إرسال واتساب', 'Send WhatsApp', NULL, 43),
  ('dashboard.view', 'reports', 'التقارير ولوحة القيادة', 'عرض لوحة القيادة', 'View dashboard', NULL, 44),
  ('dashboard.financial', 'reports', 'التقارير ولوحة القيادة', 'عرض المؤشرات المالية', 'View financial KPIs', NULL, 45),
  ('report.staff_performance', 'reports', 'التقارير ولوحة القيادة', 'تقرير أداء الموظفين', 'Staff performance report', NULL, 46),
  ('ai.query', 'ai', 'المساعد الذكي', 'استخدام المساعد الذكي', 'Use AI assistant', NULL, 47),
  ('audit.view', 'system', 'النظام', 'عرض سجل التدقيق', 'View audit log', NULL, 48),
  ('settings.manage', 'system', 'النظام', 'إدارة إعدادات المكتب', 'Manage office settings', NULL, 49),
  ('permission.manage', 'system', 'النظام', 'إدارة مصفوفة الصلاحيات', 'Manage permission matrix', NULL, 50)
